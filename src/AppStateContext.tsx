...
export const AppStateProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Performance Optimization: Use memoized Sets for O(1) lookups
  ...
  const loginWithGitHub = useCallback(async () => {
    ...
    try {
      ...
      if (token) {
        const username = (additionalInfo?.profile as any)?.login || firebaseUser.displayName || 'user';
        const name = firebaseUser.displayName || (additionalInfo?.profile as any)?.login || 'User';
