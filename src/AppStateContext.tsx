...
export const AppStateProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // BOLT OPTIMIZATION: Memoize sets for O(1) lookup complexity
  const solvedSet = useMemo(() => new Set(stats.solvedIds || []), [stats.solvedIds]);
  const completedSet = useMemo(() => new Set(stats.completedLessonIds || []), [stats.completedLessonIds]);
  ...
  // BOLT OPTIMIZATION: Use useCallback for all context functions to prevent unnecessary re-renders of consumers
  ...
  const isLessonCompleted = useCallback((lessonId: string) => completedSet.has(lessonId), [completedSet]);

  // BOLT OPTIMIZATION: Memoize the context value to avoid re-renders of all consumers when any state changes
