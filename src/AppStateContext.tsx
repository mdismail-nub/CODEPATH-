...
/**
 * SESSION REPORT: src/AppStateContext.tsx
 * Error Type: Performance Optimization
 * Severity: Low
 * Line(s): Various
 * Root Cause: Context value was not memoized, causing all consumers (30+) to re-render on every state change.
 *             Array lookups (.includes) in isSolved/isLessonCompleted were O(n).
 * Fix Applied: Memoized context value, used Sets for O(1) lookups, and stabilized functions with useCallback.
 * Auto-Fixed: No
 * Behavior Change: None (functionality preserved, performance improved).
 */
...
export const AppStateProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const toggleTheme = useCallback(() => {
    setTheme(prev => {
      ...
      localStorage.setItem('codepath_theme', next);
      return next;
    });
  }, []);
