...
export const AppStateProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // ⚡ BOLT: Memoize toggleTheme to ensure stable reference and use functional update
  const toggleTheme = useCallback(() => setTheme(prev => prev === 'light' ? 'dark' : 'light'), []);

  // ⚡ BOLT: Memoize Sets for O(1) lookups instead of O(N) array scans
  ...
  // ⚡ BOLT: Memoize helper functions to prevent unnecessary re-renders of consumers
  ...
  const checkGitHubStar = useCallback(async (): Promise<boolean> => {
    // ⚡ BOLT: We access stats.github inside checkGitHubStar, so we use a functional update
    // but checkGitHubStar is usually called after login, so we need to be careful.
    // Actually, checkGitHubStar uses stats directly, so it needs to be in deps or we use a ref.
