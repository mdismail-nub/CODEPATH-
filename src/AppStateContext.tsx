...
export const AppStateProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Performance: Memoize Sets for O(1) lookups in isSolved and isLessonCompleted
  ...
  const checkGitHubStar = useCallback(async (): Promise<boolean> => {
    // Note: We need stats.github here, but to keep the function stable,
    // we can use a ref or just accept that it might change.
    // However, stats is already a dependency of many things.
    // Let's use the functional update pattern or just use stats since it's already in the provider.
