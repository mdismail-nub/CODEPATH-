 ...
 * Line(s)       : 13, 50
 * Root Cause    : The roadmap was failing to render topics because it only looked them up by slug, while some entries in ROADMAP_STEPS used IDs.
 * Fix Applied   : Updated lookup logic to check both ID and Slug for each topic in the roadmap.
 * Auto-Fixed    : Yes
 * Behavior Change: No
 * ─────────────────────────────────────
 */
      ...
      <div className="mx-auto max-w-5xl px-6 lg:px-8">
        ...
        <div className="relative space-y-20">
          ...
          {ROADMAP_STEPS.map((step, idx) => {
            const stepTopics = step.topics.map(idOrSlug => TOPICS.find(t => t.id === idOrSlug || t.slug === idOrSlug)).filter(Boolean);
