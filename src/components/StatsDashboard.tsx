...
/**
 * SESSION REPORT: src/components/StatsDashboard.tsx
 * Error Type: Performance Optimization
 * Severity: Low
 * Line(s): 7
 * Root Cause: Total problems count was being recalculated on every render.
 * Fix Applied: Moved TOTAL_PROBLEMS calculation outside of the component.
 * Auto-Fixed: No
...
export const StatsDashboard = () => {
  ...
  }, [stats.solvedIds.length, theme]);

  const solved = stats.solvedIds.length;
  const percent = useMemo(() => Math.round((solved / TOTAL_PROBLEMS) * 100) || 0, [solved]);
